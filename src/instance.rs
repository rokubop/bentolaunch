...
//! bentolaunch again is not a request for a second copy, it is a request to see the
...
/// `Local\` scopes it to this session, so a second user gets their own bentolaunch.
const MUTEX_NAME: windows::core::PCWSTR = w!("Local\\bentolaunch_single_instance");
